...
class AIService {
  ...
  async generateDatabaseSchema(description: string): Promise<AISuggestion> {
    if (!this.openai) {
...
Please respond with a JSON structure that represents a complete database schema.
  ...
  async suggestWorkflows(project: Project, tables: Table[]): Promise<any[]> {
    if (!this.openai) {
...
- Implementation complexity (Low/Medium/High)
  ...
  private generateMockSchema(description: string): any[] {
  ...
  private getMockWorkflowSuggestions(project: Project, tables: Table[]): any[] {
    const workflows = []
  ...
  private parseWorkflowSuggestions(content: string): any[] {
