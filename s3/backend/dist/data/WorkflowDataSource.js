...
class WorkflowDataSource {
    ...
    async getByProjectId(projectId) {
        ...
        const db = this.db;
        const rows = await db.all('SELECT * FROM workflows WHERE project_id = ? ORDER BY created_at ASC', [projectId]);
        return rows.map(row => ({
            id: row.id,
    ...
    async create(projectId, input) {
        ...
        await db.run('INSERT INTO workflows (id, project_id, name, description, nodes, edges, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [id, projectId, input.name, input.description || null, '[]', '[]', now, now]);
        return {
            id,
            projectId,
            name: input.name,
    ...
    async update(id, updates) {
        ...
        if (updateFields.length > 0) {
            ...
            await db.run(`UPDATE workflows SET ${updateFields.join(', ')} WHERE id = ?`, updateValues);
