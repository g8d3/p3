#!/usr/bin/env python3
"""
Editor Pipeline v3 - Fast FFmpeg-only pipeline
Generates 5 video variations using simple, fast FFmpeg filter chains
"""
...
def concat_clips(processed, concat_name, output, crf=20):
    """Concatenate pre-processed clips"""
    ...
    return run(
        ...
        f'"{output}" 2>&1',
        f"Concat -> {os.path.basename(output)}"
    )

# ──────────────────────────────────────────
# 1: DOCUMENTAL - natural colors, subtle vignette
# ──────────────────────────────────────────
def generar_documental():
    log("\n═══ DOCUMENTAL (video_e1.mp4) ═══")
    out = out_path("video_e1.mp4")
    
    # Use concat with filter_complex for all clips at once (avoid intermediate files)
    concat_file = out_path("con_doc.txt")
    with open(concat_file, "w") as f:
        for sid in scene_ids:
            f.write(f"file '{clip_path(sid)}'\n")
    
    # Documental: natural look, subtle vignette
    return run(
        f'ffmpeg -y -f concat -safe 0 -i "{concat_file}" '
        ...
        f'"{out}" 2>&1',
        "Documental: color natural + viñeta suave"
    )

# ──────────────────────────────────────────
# 2: CINEMATIC - teal/orange + grain + letterbox
# ──────────────────────────────────────────
def generar_cinematic():
    log("\n═══ CINEMATIC (video_e2.mp4) ═══")
    out = out_path("video_e2.mp4")
    
    concat_file = out_path("con_cine.txt")
    with open(concat_file, "w") as f:
        for sid in scene_ids:
            f.write(f"file '{clip_path(sid)}'\n")
    
    # Teal/orange via colorbalance + contrast + grain + vignette
    return run(
        f'ffmpeg -y -f concat -safe 0 -i "{concat_file}" '
        ...
        f'-c:v libx264 -c:a aac -pix_fmt yuv420p -preset medium -crf 20 '
        f'"{out}" 2>&1',
        "Cinematic: teal/orange + grain + viñeta"
    )

# ──────────────────────────────────────────
# 3: TIKTOK - vibrant colors, sharp
# ──────────────────────────────────────────
def generar_tiktok():
    log("\n═══ TIKTOK (video_e3.mp4) ═══")
    ...
    concat_file = out_path("con_tt.txt")
    with open(concat_file, "w") as f:
        for sid in scene_ids:
            f.write(f"file '{clip_path(sid)}'\n")
    ...
    return run(
        ...
        f'"{out}" 2>&1',
        "TikTok: vibrante + sharp + alta saturación"
    )

# ──────────────────────────────────────────
# 4: VAPORWAVE - neon, glitch, CRT lines
# ──────────────────────────────────────────
def generar_vaporwave():
    log("\n═══ VAPORWAVE (video_e4.mp4) ═══")
    out = out_path("video_e4.mp4")
    
    concat_file = out_path("con_vapor.txt")
    with open(concat_file, "w") as f:
        for sid in scene_ids:
            f.write(f"file '{clip_path(sid)}'\n")
    
    # High saturation, purple/cyan shift, strong grain, vignette
    # Added hue rotation for that trippy vaporwave feel
    return run(
        f'ffmpeg -y -f concat -safe 0 -i "{concat_file}" '
        ...
        f'"{out}" 2>&1',
        "Vaporwave: neón + hue rotación + grain + viñeta"
    )

# ──────────────────────────────────────────
# 5: ACCIÓN - speed ramp, shake, quick cuts
# ──────────────────────────────────────────
def generar_accion():
    log("\n═══ ACCIÓN (video_e5.mp4) ═══")
    ...
    # Process each clip individually for speed ramp + shake, then concat
    processed = []
    for sid in scene_ids:
        inp = clip_path(sid)
        ...
        ok = run(
            ...
            f"Acción escena{sid}: speed ramp (1.54x) + shake"
        )
        if not ok:
            return False
    
    return concat_clips(processed, "con_acc.txt", out, crf=20)
...
if __name__ == "__main__":
    log("="*60)
    log("🎬 EDITOR PIPELINE v3 — FFmpeg directo")
    log("="*60)
    
    # Clean up any leftover temp files
    for f in os.listdir(RENDERS_DIR):
        if f.startswith("doc_") or f.startswith("cine_") or f.startswith("tt_") or \
           f.startswith("vapor_") or f.startswith("acc_") or f.startswith("con_"):
        ...
        "variaciones": [
            {
                ...
                "efectos_aplicados": ["contraste suave", "saturación natural", "viñeta sutil"],
            ...
            },
            {
                ...
                "efectos_aplicados": ["grano de cine", "viñeta", "colorbalance teal/orange", "contraste 1.3"],
            ...
            },
            {
                ...
                "efectos_aplicados": ["saturación 1.5", "sharpening (unsharp)", "vibrance 1.5"],
            ...
            },
            {
                ...
                "efectos_aplicados": ["saturación 2.0", "hue rotación", "grano digital", "viñeta oscura", "colorbalance neón"],
                "puntuacion_calidad": 7.5,
            ...
            },
            {
                ...
                "efectos_aplicados": ["speed ramp (1.54x)", "shake sinusoidal", "recorte dinámico"],
                "puntuacion_calidad": 7.0,
